...
const Toggle = ({ on, onChange }) => (
  <button
    onClick={onChange}
    className={cn('w-11 h-6 rounded-full transition-all relative flex-shrink-0', on ? 'bg-[#476D7C]' : 'bg-gray-200')}
  >
    <span className={cn('absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-all', on ? 'left-[22px]' : 'left-0.5')} />
  </button>
);
...
const Account = () => {
  ...
  const [notifications, setNotifications] = useState({ email: true, sms: false, rent: true });
    ...
    <div className="max-w-3xl mx-auto space-y-7 animate-in fade-in duration-700">
      ...
      <SectionCard title="Profile Information" subtitle="Update your personal details" icon={User}>
        <div className="flex items-center gap-5 mb-7">
          ...
          >
            <div className="w-16 h-16 rounded-2xl bg-[#476D7C] flex items-center justify-center overflow-hidden">
              {avatarSrc ? (
        ...
        <div className="flex justify-end mt-5">
          <button
            ...
            className="px-8 py-2.5 bg-[#476D7C] text-white rounded-xl text-sm font-bold hover:bg-[#5A8799] transition-all shadow-sm disabled:opacity-60 flex items-center gap-2"
          >
