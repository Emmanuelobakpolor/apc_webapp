          ...
          return (
            <button key={key} onClick={() => setActiveTab(key)}
              className={cn('flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all',
                activeTab === key ? 'bg-[#476D7C] text-white' : 'bg-white text-gray-500 border border-gray-100 hover:bg-gray-50 shadow-card')}>
              {label}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div className="w-7 h-7 border-[3px] border-gray-200 border-t-[#476D7C] rounded-full animate-spin" />
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((inquiry) => (
              <div key={inquiry.id} onClick={() => openInquiry(inquiry)}
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[#476D7C] transition-colors" />
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {loadingReplies ? (
                <div className="flex items-center justify-center py-10">
                  <div className="w-6 h-6 border-[3px] border-gray-200 border-t-[#476D7C] rounded-full animate-spin" />
                </div>
              ) : (
                <>
