      ...
      if (!res.ok) {
        ...
        throw new Error(
          msg.toLowerCase().includes('exist') || msg.toLowerCase().includes('already') || msg.toLowerCase().includes('registered')
            ? 'This email is already registered under another role (Landlord or Tenant). Each role requires a unique email address.'
            : msg
        );
      }
      setCurrentStep(3);
        ...
        <React.Fragment key={step.id}>
          <div className="flex items-center gap-2 min-w-0">
            <div className={cn(
              ...
              currentStep === step.id ? 'bg-[#002C3D] text-white border-[#002C3D]'
                : currentStep > step.id ? 'bg-[#476D7C] text-white border-[#476D7C]'
            ...
            <span className={cn(
              ...
              currentStep === step.id ? 'text-[#002C3D]'
                : currentStep > step.id ? 'text-[#476D7C]' : 'text-gray-300'
            )}>
              {step.label}
            </span>
          </div>
          {idx < steps.length - 1 && (
            <div className={cn('w-6 flex-shrink-0 h-px transition-all', currentStep > step.id ? 'bg-[#476D7C]' : 'bg-gray-200')} />
          )}
        </React.Fragment>
      ))}
    </div>
    ...
    <div className="min-h-screen bg-[#F5F7FA] flex flex-col items-center py-12 px-6 font-sans">
      ...
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-soft border border-gray-100 p-8">
              ...
              <div>
                ...
                <label className="border-2 border-dashed border-gray-200 rounded-xl p-4 flex items-center gap-3 text-gray-400 hover:border-[#476D7C]/40 hover:bg-[#EEF5F8]/40 transition-all cursor-pointer group h-[50px]">
                  ...
                  <span className="text-xs font-medium group-hover:text-[#002C3D] transition-colors">
                    {formData.cac_document ? formData.cac_document.name : 'Click to upload'}
                  </span>
                  <input type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setFormData((p) => ({ ...p, cac_document: e.target.files[0] || null }))} />
                </label>
              </div>
            </div>
            <div className="bg-amber-50 p-4 rounded-xl flex items-start gap-3 border border-amber-100">
              <Info className="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" />
              <p className="text-xs text-amber-800 font-medium leading-snug">We keep your information confidential for a trusted and smooth business experience.</p>
            </div>
            <button onClick={handleNext} className="w-full py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all">
              Next: Contact Info
            </button>
          </div>
        )}
            ...
            {isSending ? (
              ...
              <>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleNext} className="flex-[2] py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all">
        ...
        {currentStep === 3 && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div>
              <h3 className="text-base font-bold text-[#002C3D] mb-0.5">Email Verification</h3>
              <p className="text-xs text-gray-400">Please verify your email address to continue</p>
            </div>
            ...
            <div className="space-y-5">
              <div>
                <label className="label-text">Enter Verification Code</label>
                    ...
                    <input
                      ...
                      className={cn(
                        ...
                        digit ? 'border-[#002C3D] bg-[#EEF5F8] text-[#002C3D]'
                          : 'border-gray-200 bg-gray-50 text-gray-800 focus:border-[#476D7C] focus:bg-white'
                      )}
                      style={{ height: '52px' }}
                      placeholder="·"
                    />
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                onClick={handleNext}
                disabled={!isVerified}
                className="flex-[2] py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Next: Set Password
              </button>
            </div>
        ...
        {currentStep === 4 && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div>
              <h3 className="text-base font-bold text-[#002C3D] mb-0.5">Create a Password</h3>
              <p className="text-xs text-gray-400">Use at least 8 characters to keep your account secure</p>
            </div>
            <div className="space-y-5">
              <div>
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-sm p-8 flex flex-col items-center text-center relative animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-bold text-[#002C3D] mb-2">Account Created!</h3>
            <p className="text-sm text-gray-400 mb-7 leading-relaxed">Your account is ready. Head to your dashboard to add and manage your properties.</p>
            <button onClick={() => navigate('/agent/dashboard')} className="w-full py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all">
              Go to Dashboard
            </button>
          </div>
        </div>
