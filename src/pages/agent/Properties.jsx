...
// ── Image upload box (shows existing thumbnail if present) ──────────────────
      ...
      {preview ? (
        ...
        <>
          <div className={cn('rounded-xl bg-white flex items-center justify-center mb-3 shadow-card flex-shrink-0', tall ? 'w-11 h-11' : 'w-9 h-9')}>
            <Upload className={cn('text-gray-400 group-hover:text-[#002C3D] transition-colors', tall ? 'w-5 h-5' : 'w-4 h-4')} />
          </div>
...
const StepDot = ({ n, active, done }) => (
  <div className={cn(
    ...
    done || active ? 'bg-[#002C3D] border-[#002C3D] text-white' : 'bg-white border-gray-200 text-gray-400'
  )}>
...
const EmptyState = ({ onAdd }) => (
  <tr><td colSpan={8}>
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-xs text-gray-400 mb-5">Click "List Property" to add your first listing.</p>
      <button onClick={onAdd} className="flex items-center gap-2 px-5 py-2.5 bg-[#002C3D] text-white rounded-xl text-sm font-semibold hover:bg-[#003F54] transition-all">
...
const Properties = () => {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API}/${editTarget.id}/update/`, {
        method: 'PATCH', headers: authHeaders(), body: buildFormData(true),
      });
      const data = await res.json();
      if (!res.ok) { const f = Object.values(data)[0]; throw new Error(Array.isArray(f) ? f[0] : f); }
    ...
    <div className="max-w-[1400px] mx-auto space-y-7 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={openAdd} className="flex items-center gap-2 px-5 py-2 bg-[#002C3D] text-white rounded-xl text-sm font-semibold hover:bg-[#003F54] transition-all shadow-sm">
            <Plus className="w-4 h-4" /> List Property
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative animate-in zoom-in-95 duration-200">
            ...
            <div className="p-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={cn('input-field', SELECT_ARROW)}
                  >
              ...
              <div className="flex gap-3 mt-8">
                <button onClick={resetFilters} className="flex-1 py-3 border border-gray-200 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition-all">
                  Reset
                </button>
                <button onClick={applyFilters}
                  className="flex-[2] py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all flex items-center justify-center gap-2">
                  Apply Filters
                </button>
              </div>
            </div>
              ...
              <tr key={prop.id} className="hover:bg-gray-50/40 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center justify-center gap-1">
                    <button onClick={() => openEdit(prop)} title="Edit property"
                    ...
                    <button onClick={() => setViewTarget(prop)} title="View details"
            ...
            <div className="p-8">
              ...
              {addStep === 1 ? (
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <button onClick={() => setAddStep(2)} className="w-full py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all">
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={handleAddSubmit} disabled={isSubmitting}
                      className="flex-[2] py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[110] flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-sm p-8 flex flex-col items-center text-center relative animate-in zoom-in-95 duration-200">
            ...
            <div className="w-16 h-16 rounded-2xl bg-[#EEF5F8] flex items-center justify-center mb-6">
              <div className="w-10 h-10 rounded-xl bg-[#002C3D] flex items-center justify-center">
            ...
            <button onClick={() => { setIsSuccessOpen(false); openAdd(); }}
              className="w-full py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all">
            ...
            <div className="p-8">
              ...
              {editStep === 1 ? (
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <button onClick={() => setEditStep(2)} className="w-full py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all">
                    Next: Photos & Ownership
                  </button>
                </div>
              ) : (
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div>
                    ...
                    <label className="border-2 border-dashed border-gray-200 rounded-xl flex items-center gap-3 p-4 bg-gray-50 hover:bg-gray-100/60 hover:border-gray-300 cursor-pointer transition-all">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-700">{ownershipDoc ? ownershipDoc.name : editTarget.ownership_document ? 'Document uploaded — click to replace' : 'Upload ownership document'}</p>
                        <p className="text-xs text-[#476D7C] font-medium">PDF, JPG or PNG</p>
                      </div>
                      <input type="file" className="hidden" accept=".pdf,image/*" onChange={(e) => setOwnershipDoc(e.target.files[0] || null)} />
                    </label>
              ...
              <div className="relative h-56 overflow-hidden rounded-t-2xl">
                ...
                <div className="absolute bottom-4 left-6 right-16">
                  ...
                  <p className="text-white/70 text-xs flex items-center gap-1 mt-1">
                    <MapPin className="w-3 h-3" />{viewTarget.address}, {viewTarget.city_state}
                  </p>
                </div>
              </div>
            ) : (
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-[#002C3D] text-white rounded-xl text-sm font-bold hover:bg-[#003F54] transition-all flex items-center justify-center gap-2"
                >
                  <PencilLine className="w-4 h-4" /> Edit Property
                </button>
                <button
                  onClick={() => { setViewTarget(null); setDeleteId(viewTarget.id); }}
